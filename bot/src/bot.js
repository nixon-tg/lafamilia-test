...
async function showOrders(ctx){const orders=await listUserOrders(ctx.from.id);const k=new InlineKeyboard();for(const o of orders.slice(0,10))k.text(`#${o.id} • ${labels[o.status]||o.status} • ${money(o.total_cents)}`,'order:'+o.id).row();k.text('🔄 Odśwież','orders').row().text('⬅️ Cofnij','home');const text=orders.length?'📦 *Moje zamówienia*\n\nWybierz zamówienie, aby zobaczyć szczegóły.':'📦 *Moje zamówienia*\n\nNie masz jeszcze żadnych zamówień.';await safeEdit(ctx,text,k);}
async function showOrderDetails(ctx,id,back='orders'){const d=await getOrderDetails(id,ctx.from.id);if(!d){await ctx.answerCallbackQuery({text:'Nie znaleziono zamówienia.',show_alert:true});return;}const items=d.items.map(i=>`• ${i.name} × ${i.quantity} — ${money(Number(i.price_cents)*Number(i.quantity))}`).join('\n')||'Brak pozycji';const o=d.order;await safeEdit(ctx,`📦 *Zamówienie #${o.id}*\n\nStatus: *${labels[o.status]||o.status}*\nData: ${new Date(o.created_at).toLocaleString('pl-PL')}\nKwota: *${money(o.total_cents)}*\n\n*Produkty:*\n${items}`,new InlineKeyboard().text('🔄 Odśwież','order:'+o.id).row().text('⬅️ Zamówienia',back));}
...
bot.callbackQuery(/^order:(\d+)$/,async ctx=>{try{await showOrderDetails(ctx,Number(ctx.match[1]));await ctx.answerCallbackQuery();}catch(e){console.error('Order details error:',e);await ctx.answerCallbackQuery({text:'Nie udało się wczytać zamówienia.',show_alert:true});}});
...
async function showAdmin(ctx){if(!isAdmin(ctx)){if(ctx.callbackQuery)return ctx.answerCallbackQuery({text:'Brak uprawnień.',show_alert:true});return ctx.reply('Brak uprawnień.');}if(ctx.callbackQuery){await ctx.answerCallbackQuery();await safeEdit(ctx,'🛠 *Panel administratora*\n\nWybierz sekcję:',adminKeyboard());}else await ctx.reply('🛠 *Panel administratora*\n\nWybierz sekcję:',{parse_mode:'Markdown',reply_markup:adminKeyboard()});}
